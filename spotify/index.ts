...
async function getRecentlyPlayed(
  access_token: string | null | Error,
): Promise<RelevantSpotifyData> {
  if (access_token === null || access_token instanceof Error)
    return data.spotify;
...
export default async function fetchSpotifyData(): Promise<RelevantSpotifyData> {
  ...
  const access_token =
    (await getSpotifyAccessToken()) || (await refreshTokens());
